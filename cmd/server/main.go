...
func run() error {
	...
	mux := web.NewRouter(web.Handlers{
		...
		Notification: web.NotificationHandlers{
			...
			MarkRead:    handler.HandleMarkNotificationRead(notificationSvc),
			MarkAllRead: handler.HandleMarkAllNotificationsRead(notificationSvc),
		},
