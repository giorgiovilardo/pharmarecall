...
- [ ] 1.1 Initialize Go module, set up `cmd/server/main.go` entrypoint with `net/http` server
- [ ] 1.2 Set up project directory structure (`internal/`, `db/migrations/`, `static/`)
- [ ] 1.3 Add dependencies: pgx, scs, goose, templ, bcrypt
- [ ] 1.4 Set up Docker Compose with PostgreSQL 18
- [ ] 1.5 Add oat.ink CSS to `static/` and configure `embed.FS` for static assets
...
- [ ] 1.8 Configure SCS session manager with pgxstore, wire into HTTP server middleware

## 2. Login & Logout (Admin + Personnel)

- [ ] 2.1 Migration: create `users` table (id, email, password_hash, name, role, pharmacy_id nullable, created_at, updated_at)
- [ ] 2.2 Migration: create SCS sessions table (via pgxstore schema)
- [ ] 2.3 Implement password hashing/verification with bcrypt
- [ ] 2.4 Implement user repository: get by email, get by ID
...
- [ ] 2.10 Verify end-to-end: seed admin, login, see a placeholder dashboard, logout
...
- [ ] 3.2 Implement pharmacy repository: create, list (with personnel/patient counts)
- [ ] 3.3 Implement user repository: create user with role
...
- [ ] 3.8 Verify end-to-end: admin logs in, sees pharmacy list, creates pharmacy with owner, owner can log in
...
- [ ] 5.1 Migration: create `patients` table (id, pharmacy_id, first_name, last_name, phone, email, delivery_address, fulfillment, notes, consensus, consensus_date, created_at, updated_at)
- [ ] 5.2 Implement patient repository: create, list by pharmacy, get by ID, update
...
- [ ] 6.1 Migration: create `prescriptions` table (id, patient_id, medication_name, units_per_box, daily_consumption, box_start_date, created_at, updated_at)
- [ ] 6.2 Migration: create `refill_history` table (id, prescription_id, box_start_date, box_end_date, created_at)
- [ ] 6.3 Implement prescription repository: create, list by patient, get by ID, update
...
- [ ] 6.9 Implement refill recording: update box_start_date, insert previous period into refill_history
...
- [ ] 7.1 Migration: create `order_statuses` table (id, prescription_id, status, created_at, updated_at)
- [ ] 7.2 Implement dashboard query: join prescriptions + patients, calculate depletion, fetch order statuses, filter by pharmacy
- [ ] 7.3 Create dashboard page with entries grouped by prescription status (depleted first, then approaching, then ok)
- [ ] 7.4 Display per entry: patient name, medication, depletion date, days remaining, status badge, fulfillment method, order status
- [ ] 7.5 Implement prescription status filter (all, depleted, approaching, ok)
- [ ] 7.6 Implement date range filter
- [ ] 7.7 Implement order status filter (all, pending, prepared, fulfilled)
- [ ] 7.8 Implement order status advancement from dashboard: pending → prepared → fulfilled (form POST)
- [ ] 7.9 Implement configurable lookahead window (default 7 days)
- [ ] 7.10 Verify end-to-end: personnel sees dashboard with prescriptions, filters work, advances order status through full lifecycle

## 8. Personnel Notifications

- [ ] 8.1 Migration: create `notifications` table (id, pharmacy_id, prescription_id, transition_type, read, created_at)
- [ ] 8.2 Implement notification repository: create, list by pharmacy, mark read, mark all read, count unread
